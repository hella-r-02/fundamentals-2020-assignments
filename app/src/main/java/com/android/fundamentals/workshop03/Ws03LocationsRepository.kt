...
class Ws03LocationsRepository(applicationContext: Context) : LocationRepository {
    ...
    private val dataBase = Ws03DataBase.create(context = applicationContext)
    ...
    override suspend fun getAllLocations(): List<Location> = withContext(context = Dispatchers.IO) {
        dataBase.locationsDao.getAllLocations().map { toLocation(it) }
    }
    ...
    override suspend fun addNewAndGetUpdated(): List<Location> = withContext(Dispatchers.IO) {
        ...
        dataBase.locationsDao.insertLocation(toEntity(newLocationRequest))
        getAllLocations()
    }
    ...
    override suspend fun deleteByIdAndGetUpdated(id: Long): List<Location> =
        withContext(Dispatchers.IO) {
            dataBase.locationsDao.deleteLocationById(id)
            getAllLocations()
        }
